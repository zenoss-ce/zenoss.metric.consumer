...
public class MetricWebSocketTest {
    ...
    public void testOnMessage() throws Exception {
        MetricWebSocket socket = new MetricWebSocket(config(false), service);
        when(service.push(anyListOf(Metric.class), anyString(), any(BufferListener.class))).thenReturn(new Control());
        ...
        verify(service).push(eq(Collections.singletonList(metric)), eq("websocket1"), any(BufferListener.class));
    }
    ...
    public void testOnMessageInjectsTags() throws Exception {
        ...
        MetricWebSocket socket = new MetricWebSocket(config(prefixes, null, true), service);
        ...
        verify(service).push(eq(Collections.singletonList(expected_metric)), eq("websocket1"), any(BufferListener.class));
    }
    ...
    public void testOnMessageFiltersTags() throws Exception {
        ...
        MetricWebSocket socket = new MetricWebSocket(config(prefixes, whiteList, true), service);
